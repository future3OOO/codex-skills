#!/usr/bin/env bash
# Port selected codex-side improvements BACK to claude-skills.
...
set -euo pipefail
cd "$(dirname "$0")/.."
[[ $# -eq 0 ]] && { echo "usage: $0 <codex-side paths...>"; exit 1; }

work=$(mktemp -d)
git clone --quiet https://github.com/future3OOO/claude-skills.git "$work"
branch="codex-sync/$(date +%Y%m%d-%H%M%S)"
git -C "$work" switch -c "$branch" --quiet

for f in "$@"; do
  [[ -f "$f" ]] || { echo "skip (not a file): $f"; continue; }
  dest=$(python3 -c "import sys; sys.path.insert(0,'scripts'); from estate_xform import _inverse_path; p='$f'; p=_inverse_path(p,'codex'); print(p)")
...
git -C "$work" commit --quiet -m "Port codex-skills improvements back to claude estate"
