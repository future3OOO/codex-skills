...
from .behavior_map import initial_items, interpretation_pending
...
def validate_document(
    value: object, *, require_behavior_map: bool = False, allow_unsettled: bool = False,
) -> dict[str, object]:
    ...
    non-empty. Open questions refuse unless the producer retains a structurally
    valid unsettled interpretation as pending evidence.
    ...
    if document["openQuestions"] != "none" and not (
        allow_unsettled and any(interpretation_pending(item) for item in document.get(BEHAVIOR_MAP_SECTION, []))
    ):
...
def validated_document(path: str) -> dict[str, object]:
    ...
    return validate_document(value, require_behavior_map=True, allow_unsettled=True)
