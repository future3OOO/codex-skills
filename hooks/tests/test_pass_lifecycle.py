...
class PassLifecycleTests(unittest.TestCase):
    ...
    def rewrite_latest_state(self, update) -> None:
        ...
        deliberately damage the active workflow's stored state, then assert
    ...
    def advise(self, slug: str, wid: str, stage: str = "preflight", verdict: str = "completed",
               material: bool = False) -> subprocess.CompletedProcess[str]:
        """Record an advisor consult raising one nonbehavioral finding, SPEC-1."""
        self.documents += 1
        envelope = self.tmp / f"advice-{self.documents}.json"
        envelope.write_text(json.dumps({"schemaVersion": 1, "verdict": verdict, "findings": [
            {"id": "SPEC-1", "claim": "the fold could bypass completion", "material": material, "kind": "nonbehavioral"}]}))
    ...
    def test_summary_keeps_a_large_map_and_the_verification_command(self) -> None:
        # The biggest recorded maps (38 items in live ledgers) still fit with every open
        # item named, settled items omitted, the closing invariant and the verification command.
        wid = self.begin_slug("summary-large-map")
        self.advance_to_context_forge()
        self.run_cli(
            ("record", "advisor-result", "--slug", "summary-large-map", "--workflow-id", wid, "--stage", "preflight", "--source", "codex-advisor", "--verdict", "completed"),
    ...
    def test_checkpoint_is_the_complete_advisor_stage_descriptor(self) -> None:
        ...
        projection = checkpoint.get("advisor-projection")
        self.assertEqual(
            (
                ...
                next((c["evidenceId"] for c in checkpoint.get("channels", []) if c["name"] == "advisor-projection"), None),
    ...
    def ledger_rows(self) -> tuple[tuple[object, ...], ...]:
        tables = ("ledger_metadata", "workflows", "evidence", "review_manifests", "workflow_events",
                  "event_evidence", "event_manifests", "evidence_parts")
    ...
    def test_candidate_reporting_commands_cannot_commit_stale_candidate(self) -> None:
        ...
        self.owner_phase("tdd", "not-required")
        self.assert_candidate_race_refused(["pause", "--reason", "racing receipt",
    ...
    def test_a_shell_mutation_after_review_refuses_the_final_recording(self) -> None:
        ...
        refused = self.cli(
            ...
            "--stage", "final", "--source", "codex-advisor", "--input", commit_ready_envelope(self.tmp),
        )
        self.assertEqual(refused.returncode, 2, refused.stdout + refused.stderr)
    ...
    def test_preflight_records_only_with_its_document(self) -> None:
        ...
        self.assertIn("producers record every other step", bare.stderr, "the refusal did not name the producers")
        self.assertEqual(json.loads(self.cli("status").stdout)["preflight"], "pending")

        payload = self.tmp / "preflight-input.json"
        payload.write_text('{"authoritativeContract": "text", "behaviorMap": [], '
                           '"authoritativeContract": "repeated"}', encoding="utf-8")
        ...
        self.assertIn("repeats field", duplicated.stderr)
        self.assertIn("authoritativeContract", duplicated.stderr)
    ...
    def test_a_fix_round_demands_fresh_evidence_and_tdd_waits_for_preflight_evidence(self) -> None:
        ...
        self.advance_to_context_forge()
        self.run_cli(
            ("record", "advisor-result", "--slug", "fresh-evidence", "--workflow-id", wid, "--stage", "preflight", "--source", "codex-advisor", "--verdict", "completed"),
        ...
        self.advance_to_context_forge()
        self.run_cli(
            ("record", "advisor-result", "--slug", "fresh-evidence", "--workflow-id", new_wid, "--stage", "preflight", "--source", "codex-advisor", "--verdict", "completed"),
    ...
    def test_tdd_demands_preflight_evidence_not_just_status(self) -> None:
        ...
        self.advance_to_context_forge()
        self.run_cli(
            ("record", "advisor-result", "--slug", "bare-preflight-tdd", "--workflow-id", wid, "--stage", "preflight", "--source", "codex-advisor", "--verdict", "completed"),
    ...
    def test_exit_codes_reflect_the_recording_not_the_reporting(self) -> None:
        ...
        self.advance_to_context_forge()
        self.run_cli(
            ("record", "advisor-result", "--slug", "exit-honesty", "--workflow-id", wid, "--stage", "preflight", "--source", "codex-advisor", "--verdict", "completed"),
    ...
    def test_workflow_completion_survives_a_same_tree_review_commit(self) -> None:
        ...
        final = self.cli(
            ...
            "--input", commit_ready_envelope(self.tmp),
        )
        ...
        self.assertEqual({key: state["finalReview"][key] for key in ("findings", "source", "status")},
                         {"findings": "none", "source": "codex-advisor", "status": "commit-ready"})
        self.assertTrue(state["finalReview"]["intakeEvidence"], "a completed final review has no recorded envelope")
    ...
    def test_public_phase_updates_follow_order_and_cannot_bypass_owned_producers(self) -> None:
        ...
        for phase in ("repo-context-forge", "tdd", "code-review"):
            ...
            self.assertIn("producers record every other step", shortcut.stderr)
    ...
    def test_a_bare_context_forge_claim_publishes_as_pending_everywhere(self) -> None:
        ...
        self.assertIn("repoContextForge", self.cli("complete").stderr)
    ...
    def test_the_retired_gitnexus_transition_refuses_as_an_obsolete_step(self) -> None:
        ...
        for identity in ((), ("--slug", "obsolete-gitnexus", "--workflow-id", before["workflowId"])):
            ...
            self.assertEqual(refused.returncode, 2, refused.stdout + refused.stderr)
            self.assertIn("producers record every other step", refused.stderr)
        self.assertEqual(json.loads(self.cli("status").stdout), before)
    ...
    def test_implementation_and_reviews_wait_for_green(self) -> None:
        ...
        early_final = self.cli(
            "record", "advisor-result", "--slug", "tdd-gates", "--workflow-id", wid, "--stage", "final", "--source", "codex-advisor", "--input", commit_ready_envelope(self.tmp),
        )
    ...
    def test_advisor_disposition_cannot_create_or_alter_raw_results(self) -> None:
        ...
        self.assertIn("unrecognized arguments: --findings", direct.stderr)

        recorded = self.advise("producer-owned-advice", wid)
        self.assertEqual(recorded.returncode, 0, recorded.stdout + recorded.stderr)
    ...
    def test_context_mismatch_invalidation_preserves_reopened_gates(self) -> None:
        ...
        self.assertEqual(
            (after["phase"], after["verification"], after["codeReview"]["status"], after["finalReview"]["status"]),
            ("implementation", "pending", "pending", "pending"), "CONTEXT_MISMATCH_INVALIDATION_GATES_REGRESSED")
    ...
    def test_terminal_context_mismatch_allows_reconsult(self) -> None:
        ...
        wid = self.begin_slug(slug); self.advance_to_verification(slug, wid)
        self.owner_phase("code-review", "passed", findings="none"); self.run_cli(("record", "advisor-result", "--slug", slug, "--workflow-id", wid, "--stage", "final", "--source", "codex-advisor", "--input", commit_ready_envelope(self.tmp)), ("record", "advisor-disposition", "--slug", slug, "--workflow-id", wid, "--stage", "final", "--findings", "none"))
    ...
    def test_open_correction_batch_blocks_broad_gates_and_routes_tdd_reassessment(self) -> None:
        ...
        recorded = self.cli("record", "advisor-result", "--slug", slug, "--workflow-id", wid, "--stage", "final",
                            "--source", "codex-advisor", "--input", str(envelope))
        self.assertEqual(recorded.returncode, 0, recorded.stderr); state = json.loads(self.cli("status").stdout)
    ...
    def test_a_disposition_document_answers_only_for_its_own_stage_and_instance(self) -> None:
        ...
        self.advance_to_context_forge()
        self.run_cli(
            ("record", "advisor-result", "--slug", "disposition-lifetime", "--workflow-id", reused,
             "--stage", "preflight", "--source", "codex-advisor", "--verdict", "completed"),
            ("record", "advisor-disposition", "--slug", "disposition-lifetime", "--workflow-id", reused,
             "--stage", "preflight", "--findings", "none"),
        )
        self.assertNotIn(
            "dispositionEvidence", json.loads(self.cli("status").stdout)["advisorPreflight"],
    ...
    def test_identical_pending_preflight_design_replay_is_a_no_op(self) -> None:
        ...
        self.advance_to_context_forge()
        first = self.cli(
            "record", "advisor-result", "--slug", "design-replay", "--workflow-id", wid,
            "--stage", "preflight", "--source", "codex-advisor", "--verdict", "completed",
        )
    ...
    def test_advisor_results_bind_to_the_workflow_instance(self) -> None:
        ...
        self.assertEqual(
            ...
            "a delayed consult mutated the new workflow instance",
        )
    ...
    def test_completed_state_is_terminal_until_governance_revalidation(self) -> None:
        ...
        for mutation in (
            ("record", "advisor-result", "--slug", "terminal-state", "--workflow-id", wid, "--stage", "final", "--source", "codex-advisor", "--input", commit_ready_envelope(self.tmp)),
...
"""
        ...
        final = self.cli("record", "advisor-result", "--slug", "terminal-state", "--workflow-id", wid, "--stage", "final", "--source", "codex-advisor", "--input", commit_ready_envelope(self.tmp))
        self.assertEqual(final.returncode, 0, final.stdout + final.stderr)
    ...
    def test_optional_lead_identity_is_validated_against_the_active_instance(self) -> None:
        ...
        for label, transition in (
            ("pause", ("pause", "--reason", "stale", "--slug", "lead-identity", "--workflow-id", stale_wid)),
        ...
        for label, transition in (
            ("pause", ("pause", "--reason", "stale", "--slug", "lead-identity-replacement", "--workflow-id", stale_wid)),
        ...
        self.assertNotIn("paused", state, "a stale lead command advanced the replacement workflow")
    ...
    def test_rearm_adapter_restores_only_recorded_pass_state(self) -> None:
        ...
        self.assertNotIn("repo-context-forge=", rearmed.stdout)
        self.assertIn("advisor-preflight=pending", rearmed.stdout)
        self.assertIn("final-review=pending/pending", rearmed.stdout)
    ...
    def test_completion_requires_a_ready_final_review_and_resolved_findings(self) -> None:
        ...
        for verdict in ("fix-before-commit", "commit-ready"):
            before = len(self.history_events())
            raw = self.cli("record", "advisor-result", "--slug", "completion-contract", "--workflow-id", wid,
                           "--stage", "final", "--source", "codex-advisor", "--verdict", verdict)
            self.assertEqual((raw.returncode, len(self.history_events())), (2, before),
                             "RAW_FINAL_VERDICT_ACCEPTED" + raw.stdout + raw.stderr)
            self.assertIn("envelope", raw.stderr, "RAW_FINAL_VERDICT_ACCEPTED")
        ...
        envelope.write_text('{"schemaVersion":1,"findings":[],"verdict":"commit-ready"}', encoding="utf-8")
        ready = self.cli("record", "advisor-result", "--slug", "completion-contract", "--workflow-id", wid, "--stage", "final", "--source", "codex-advisor", "--input", str(envelope))
        self.assertEqual(ready.returncode, 0, ready.stdout + ready.stderr)
        self.assertEqual(self.cli("complete").returncode, 0, "an enveloped commit-ready final did not complete")
