...
class ABEstateBenchmarkTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # CODEX_HOME deliberately points at a scratch directory, the way
        ...
        # that authored config.toml. Anchoring the estate prefix on Path.home()
        ...
        self.env = {
            **os.environ,
            ...
            "CODEX_HOME": str(self.tmp / "scratch-home"),
    ...
    def declared_estate(self) -> str:
        ...
        value = json.loads((self.clone / "config.toml").read_text(encoding="utf-8"))
    ...
    def test_arms_built_from_one_ref_report_parity_isolation_and_exact_identities(self) -> None:
        ...
        self.assertIn(str(Path(self.env["HOME"]) / ".codex"), isolation["liveEstates"])
        self.assertIn(self.env["CODEX_HOME"], isolation["liveEstates"])
        for arm in (baseline, candidate):
            written = (Path(arm["home"]) / "config.toml").read_text(encoding="utf-8")
    ...
    def test_an_out_path_inside_a_protected_root_is_refused_before_anything_is_deleted(self) -> None:
        ...
        protected = Path(self.env["CODEX_HOME"])
    ...
    def test_an_arm_trace_left_under_a_monitored_state_root_fails_isolation(self) -> None:
        ...
        root = Path(self.env["CODEX_HOME"]) / "state"
    ...
    def test_a_candidate_that_escapes_its_state_root_is_caught(self) -> None:
        ...
        escape = Path(self.env["CODEX_HOME"]) / "state"
        ...
        perturbed = original.replace(
            ...
            '    override = os.environ.get("BENCHMARK_ESCAPE_ROOT") or os.environ.get("CODEX_WORKFLOW_STATE_ROOT")')
