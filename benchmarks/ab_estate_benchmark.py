...
SCHEMA_VERSION = 1
REPETITIONS = 5
ESTATE = ("AGENTS.md", "config.toml", "hooks", "skills")
...
REPLAY = (
    ...
    ("replay-advisor-final", lambda c: [[*c["cli"], "advisor-result", *c["bound"], "--stage", "final",
                                        ...
                                        "--design-declaration", str(c["inputs"] / "design-absent.json")]],
     _fields(STATE_FIELDS), lambda p: p["finalReview.source"] == "claude-advisor" and p["finalReview.status"] == "commit-ready"),
...
def declared_estates(settings: Path) -> set[Path]:
    ...
    README records that config.toml hardcodes one absolute install path, so the
...
def monitored(source: Path) -> tuple[list[Path], list[Path]]:
    ...
    homes = {installed, Path(os.environ.get("CODEX_HOME", installed)).expanduser()}
    homes |= declared_estates(source / "config.toml")
    roots = {home / "state" for home in homes}
    if override := os.environ.get("CODEX_WORKFLOW_STATE_ROOT"):
        ...
        for entry in group:
            ...
            for hook in entry.get("hooks", []):
                ...
                if all(Path(token).exists() for token in absolute):
                    kept.append(hook)
            entry["hooks"] = kept
    (home / "config.toml").write_text(json.dumps(value, indent=2) + "\n", encoding="utf-8")


def arm_env(root: Path, state_root: Path) -> dict[str, str]:
    # HOME as well as CODEX_HOME: the estate's codex_home() falls back to Path.home().
    return {**os.environ, "HOME": str(root), "CLAUDE_CONFIG_DIR": str(root / "home"),
            "CODEX_HOME": str(root / "home"), "CODEX_WORKFLOW_STATE_ROOT": str(state_root),
...
def build_arm(source: Path, out: Path, name: str, ref: str, commit: str) -> dict[str, object]:
    ...
    for item in ("AGENTS.md", "hooks", "skills"):
        target = clone / item
