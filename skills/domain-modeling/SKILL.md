...
Merely reading `CONTEXT.md` for vocabulary is not this skill. That is a one-line habit any skill can follow. Use this skill when changing the model, not just consuming it.
...
When a term is resolved, update `CONTEXT.md` immediately. Do not batch these up; capture them as they happen. Use the format in [CONTEXT-FORMAT.md](CONTEXT-FORMAT.md).

`CONTEXT.md` should be devoid of implementation details. Do not treat it as a spec, a scratch pad, or a repository for implementation decisions. It is a glossary and nothing else.
...
2. **Surprising without context** - a future reader will wonder why it works this way.
