...
Correct a mistaken RED contract command with the existing disposition: `{"id":"BM_ATTACK","status":"pending","evidence":"The command selected the wrong occurrence; the contract is unchanged."}`. This releases only that item's current command/proof binding and matching active cycle; its contract, finding references and historical RED proof remain. The previously attacked item still cannot be withdrawn. Run the corrected command through ordinary RED, then matching GREEN. Correction supplies no proof and pending still blocks completion; unrelated items and receipts need no replay. Already-proved contracts cannot be reset this way.
