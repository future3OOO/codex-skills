...
`IntakeSerialisationTests.test_higher_slots_count_during_competing_admissions`.
Its fake two-permit memory source must use a private slot root for both the
held higher slot and the loaded admission module. Production account-wide
admission, limits, lock lifetimes and other tests are unchanged.

**Evidence:** A real foreign flock acquired at the test's release boundary
reproduced `1 != 2` on unchanged `071e4bf`. The restored fixture passes under
the same interference. This proves test isolation, not a runtime admission fix.

**Reconciliation:** Incorporate `docs/decisions-delivery-record` at `0da3b493`
as a merge parent without importing its obsolete PR62 matcher narrative.
Preserve its unrelated delivery history and the current read-capture repair
record. The dropped narrative's 239/239, event-stream, probe-file and advisor
claims do not describe this candidate. No merge to main or installation is
included; #59 D and paired-compaction acceptance remain outside this repair.
...
**Reconciliation (2026-09-18):** The PR62 matcher/review narrative formerly
following this entry is superseded by the Sep-18 repair record above. At the
maintainer's direction it is dropped, including rounds 4-7 and their discarded
implementation/approval claims; it is not merged as current delivery. Other
entries from `docs/decisions-delivery-record` at `0da3b493` are retained as
historical reports, not fresh verification. The CLI token figures above compare
different workflow snapshots and are not a controlled before/after measurement.
